    ...
    public class LabTechnicianController : ControllerBase
    {
        ...
        public async Task<IActionResult> ReportId([FromBody] TestReportNew report)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var Id = await _lab.CreateReport(report);
                    if (Id > 0)
                    {
                        return Ok(Id);
                    }
        ...
        #region add lab report

        [HttpPost]
        [Route("labreport")]
        public async Task<IActionResult> LabReport([FromBody] Reportlist report)
        {
            //check validation of body
            if (ModelState.IsValid)
            {
                try
                {
                    var Id = await _lab.LabReport(report);
                    if (Id > 0)
                    {
                        return Ok(Id);
                    }
