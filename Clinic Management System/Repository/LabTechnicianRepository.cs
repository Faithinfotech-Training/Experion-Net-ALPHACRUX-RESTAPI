...
namespace Clinic_Management_System.Repository
{
    public class LabTechnicianRepository:ILabTechnicianRepository
    {
        ...
        public async Task<int> CreateReport(TestReportNew report)
        {
            if (_db != null)
            {
                await _db.TestReportNew.AddAsync(report);
                await _db.SaveChangesAsync();
        ...
        public async Task<int> LabReport(Reportlist report)
        {
            if (_db != null)
            {
                await _db.Reportlist.AddAsync(report);
                await _db.SaveChangesAsync();
                return report.ReportlistId;
            }
            return 0;
